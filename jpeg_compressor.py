...
def entropy_encode(data):
    ...
    probabilities = {byte: freq / total_count for byte, freq in frequencies.items()}
...
#     return round(old_size/new_size, 3)
# Modified
def compress_color(input_filename, output_filename, block_size, quality_factor, verbose):
    """
    Main compression algorithm for color pics.
    ...
    dc_coefficients = []
    height, width = ims[0].shape
    for level in range(3):  # For each color channel
        im, temp_dc_coefficients, temp_zags = matrix_to_zags(ims[level], block_size, quality_factor)
        ims[level] = im
        zags.extend(temp_zags)
        dc_coefficients.extend(temp_dc_coefficients)
    ...
    huffman_encoded_data = huffman_encode(zags)
    ...
    if verbose:
        print(f"Compressed {input_filename} to {output_filename} using Huffman encoding")
        print(f"Block size {block_size} and quality factor {quality_factor}")
        print(f"Took {round(time.time() - start_time, 2)} seconds")
        print(f"Original file size: {old_size} bytes")
        print(f"New file size: {new_size} bytes")
...
def compress_bw(input_filename, output_filename, block_size, quality_factor, verbose):
    """
    Main compression algorithm for b&w pics.
    ...
    start_time = time.time()
    im_file = Image.open(input_filename)
    im = np.float32(im_file.convert('L'))  # Convert to grayscale
    if verbose:
        print(f"Opened file {input_filename} for compression")
...
if __name__=="__main__":
    input_file = "/Users/kunikameshram/Documents/Project/JPEG compression/jpeg_compresser/Kodak08.bmp"  
    output_file = "/Users/kunikameshram/Documents/Project/JPEG compression/jpeg_compresser/output.jpeg"  
    block_size = 8  # block size
    quality_factor = 50  # quality factor
    compress_color(input_file, output_file, block_size, quality_factor, verbose=True)
